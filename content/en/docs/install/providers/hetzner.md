...
To learn why Cozystack developers made a decision to use Talos as the foundation of a Cozystack cluster,
...
1.  Unmount the cloud-init partition, sync changes, and reboot the server:
...
At this point, Talos is installed on the nodes and is waiting in the maintenance mode.
...
1.  Make a directory for cluster configuration and intialize a Talm project in it.
...
1.  Create node configuration files from templates and values:
    ...
    This guide assumes that you have only three dedicates servers, so they all must be control plane nodes.
    If you have more and want to separate control plane and worker nodes, use `templates/worker.yaml` to produce worker configs:

    ```bash
    taml template -e 12.34.56.104 -n -e 12.34.56.104 -t templates/worker.yaml -i > nodes/worker1.yaml
    ```
...
1.  Once the configuration files are ready, apply them to each node:
    ...
    This command initializes nodes, setting up authenticated connection.
...
1.  Pass the token to RobotLB in order to create a load balancer in Hetzner.
